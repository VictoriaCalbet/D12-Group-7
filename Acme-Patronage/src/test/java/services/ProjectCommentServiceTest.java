...
public class ProjectCommentServiceTest extends AbstractTest{
	...
	//Supporting services
		...
		/***
		 * 
		 * Requirement 21.4. An actor who is not authenticated must be able to list the comments of a project.
		...
		/***
		 * 
		 * Requirement 23.3. An actor authenticated as a user must be able to post a comment in a project, even if he or she hasn't funded it.
		 * 
		 * Testing cases:
		 * 
		...
		public void postProjectComment() {
			...
			Project project = this.projectService.findOne(this.getEntityId("project1"));
		...
		protected void postProjectCommentsUser(final String text, Integer rating, Date cMoment, String user, Project project, final Class<?> expectedException) {
			Class<?> caught = null;

			try {
				this.authenticate(user);

				ProjectComment aC = this.projectCommentService.create();

...
public class AnnouncementCommentServiceTest extends AbstractTest{
	...
	//Service being tested
		...
		//Supporting services
			...
			/***
			 * 
			 * Requirement 21.6. An actor who is not authenticated must be able to list the comments of an announcement.
			...
			public void listAnnouncementComments() {
				...
				int announcementId = this.getEntityId("announcement2");
			...
			/***
			 * 
			 * Requirement 23.5. An actor authenticated as a user must be able to post a comment in an announcement, only if he or she has funded the
			 * related project.
			 * 
			 * Testing cases:
			 * 
			...
			public void postAnnouncementComment() {
				...
				Announcement announcement = this.announcementService.findOne(this.getEntityId("announcement1"));
			...
			protected void postAnnouncementCommentsUser(final String text, Integer rating, Date cMoment, String user, Announcement announcement, final Class<?> expectedException) {
				Class<?> caught = null;

				try {
					this.authenticate(user);

					AnnouncementComment aC = this.announcementCommentService.create();
